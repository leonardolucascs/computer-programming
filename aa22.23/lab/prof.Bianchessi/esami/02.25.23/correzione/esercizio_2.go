...
// Interseca multiInsiemi e resituisci risultato
...
func StampaMultiInsieme(A map[int]int) {
	if len(A) <= 0 {
		...
		// Inizializzazione slice
		slKeys := []int{}

		// salva chiavi per ordinarle in seguito
		for k, _ := range A {
